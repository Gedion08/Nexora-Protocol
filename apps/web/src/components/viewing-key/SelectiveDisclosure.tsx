...
export function SelectiveDisclosure() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      let proof = "0x" + Array(64).fill(0).map(() => Math.floor(Math.random() * 16).toString(16)).join("");
      let publicInputs = ["0x" + Array(32).fill(0).map(() => Math.floor(Math.random() * 16).toString(16)).join("")];

      if (isProverConfigured()) {
        const realProof = await generateDisclosureProof({
          type: disclosureType,
          viewingKey: {
            publicKey: selectedKey.publicKey,
            viewingKey: selectedKey.viewingKey,
          },
          fields: selectedFields,
          threshold,
          operator,
        ...
        if (realProof) {
          ...
          publicInputs = realProof.publicInputs;
        }
      }
      ...
      setMessage({
        type: "success",
        text: isProverConfigured()
          ? `${DISCLOSURE_TYPES.find((t) => t.value === disclosureType)?.label ?? "Disclosure"} proof generated by prover.`
          : `${DISCLOSURE_TYPES.find((t) => t.value === disclosureType)?.label ?? "Disclosure"} proof generated successfully.`,
      });
    } catch (err) {
      setMessage({
        type: "error",
        text: err instanceof Error ? err.message : "Failed to generate disclosure proof",
      });
    } finally {
