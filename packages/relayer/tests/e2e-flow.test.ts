...
describe('E2EOrchestrator', () => {
  ...
  describe('initialize', () => {
    it('should initialize the privacy hub client', async () => {
      await orchestrator.initialize();
      expect(mockPrivacyHub.initialize).toHaveBeenCalled();
    });
  });
  ...
  describe('getIntentStatus', () => {
    ...
    it('should return null for shieldTx when no swap', async () => {
      mockIntentRepo.getById = vi.fn().mockResolvedValue({
        id: 'intent-1',
        status: 'pending',
        created_at: '2024-01-01',
      ...
      mockSwapRepo.getByIntentId = vi.fn().mockResolvedValue(null);
      mockDepositRepo.getByIntentId = vi.fn().mockResolvedValue([]);

      const result = await orchestrator.getIntentStatus('intent-1');
      expect(result).not.toBeNull();
      expect(result!.shieldTx).toBeUndefined();
    });
  });
  ...
  describe('getHealth', () => {
    ...
    it('should return bridge unhealthy when checkHealth throws', async () => {
    ...
    it('should return fallback bridge unhealthy when checkHealth throws', async () => {
